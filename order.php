<?php
// Uncomment below lines for debugging only — never enable on production
...
if ($task === 'order') {
    ...
    if ($action === 'upate_invoice') {
        ...
        if ($_POST) {
            ...
            $result = update_order_data($order_id, $customer_id, $staff_id, $branch_id, $subtotal, $discount, $total, $warrenty, $model_text, $model_price, $customer, $items);
    ...
    if (isset($_POST['imei']) && '' !== $_POST['imei']) {
        $data = getProductDataByImei($_POST['imei'], $_SESSION['branch_id'] ?? 0, 1);
...
if ($task === 'save') {
    ...
    if ($_POST) {
        ...
        $customer    = json_decode($_POST['customer'] ?? '{}');
        $items       = json_decode($_POST['items']    ?? '[]');
        $result      = save_order_data($staff_id, $branch_id, $subtotal, $discount, $total, $warrenty, $model_text, $model_price, $customer, $items);
        echo json_encode(['result' => $result]);
    }
    exit;
}
...
// ─── Default: show order page ─────────────────────────────────────────────────
$smarty_appform->display('order/product_order.tpl');
exit;
