...
function smarty_function_paginate_next($params, &$smarty) {
    ...
    if(($_item = (new SmartyPaginate)->_getNextPageItem($_id)) !== false) {
        $_show = true;
        $_text = isset($params['text']) ? $params['text'] : (new SmartyPaginate)->getNextText($_id);
        $_url .= (strpos($_url, '?') === false) ? '?' : '&';
        $_url .= (new SmartyPaginate)->getUrlVar($_id) . '=' . $_item;
    } else {
        $_show = false;   
    }
