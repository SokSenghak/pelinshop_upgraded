...
function smarty_function_paginate_last($params, &$smarty) {
    ...
    if ((new SmartyPaginate)->getTotal($_id) === false) {
        $smarty->trigger_error("paginate_last: total was not set");
        return;        
    }
    
    $_url = (new SmartyPaginate)->getURL($_id);
    $_total = (new SmartyPaginate)->getTotal($_id);
    $_limit = (new SmartyPaginate)->getLimit($_id);
    
    $_attrs = !empty($_attrs) ? ' ' . implode(' ', $_attrs) : '';    
    
    $_text = isset($params['text']) ? $params['text'] : (new SmartyPaginate)->getLastText($_id);
    $_url .= (strpos($_url, '?') === false) ? '?' : '&';
    $_url .= (new SmartyPaginate)->getUrlVar($_id) . '=';
