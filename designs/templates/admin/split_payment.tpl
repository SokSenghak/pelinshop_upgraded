                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            <table class="table">
                                <tbody>
                                    <tr>
                                        <th style="width:320px;"> Total (ទឹកប្រាក់នៅជំពាក់) </th>
                                        <td>
                                            <span> {($total_invoice_customer.sub_total - $total_invoice_customer.total_payment)|@number_format:2:".":","} </span> $
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
            ...
            <div class="panel-body">
					  		...
					  		<div class="row">
          						<div class="col-md-6">
									<div class="form-group">
										<label for="pwd">កាលបរិច្ឆេទ (Date) <span style="color:red"> * </span> </label>
										<div class="input-group date">
											<input type="text" id="date_pay" name="date_pay" class="form-control" placeholder="កាលបរិច្ឆេទបញ្ជាបង់(Pay Date)" value="{if $smarty.session.spayment.date_pay}{$smarty.session.spayment.date_pay|escape}{/if}" autocomplete="off" />
								...
								<div class="col-md-6">
                                    <div class="form-group">
                                        <label for="pwd">ទឹកប្រាក់បង់ (Payment) <span style="color:red"> * </span></label>
                                        <div class="input-group">
                                            <span class="input-group-addon">$</span>
                                            <input type="text" name="total" class="form-control" placeholder="ចំនួនទឹកប្រាក់បង់ (Pay Amount)" value="{if $smarty.session.spayment.total}{$smarty.session.spayment.total|escape}{/if}" onkeyup="NumAndTwoDecimals(event , this);">
                                        </div>
                                        <span style="color:red">{if $error.dollar eq 1}សូមបញ្ចូលចំនួនត្រូវបង់។ (Please enter total.){/if}</span>
                                        <span style="color:red">{if $error.dollar_over eq 1}ចំនួនទឹកប្រាក់បង់បានលើស ចំនួនទឹកប្រាក់នៅជំពាក់! (The amount paid exceeds the amount owed!){/if}</span>
                                    </div>
								</div>
							</div>
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" name="note" rows="3" id="note">{if $smarty.session.spayment.note}{$smarty.session.spayment.note|escape}{/if}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th>សរុបដុល្លារ <br> Total Dollar</th>
                                <th>ថ្ងៃបង់ប្រាក់ <br > Pay Date</th>
                                <th>ចំណាំ <br > Note</th>
                                <th>សកម្មភាព <br> Action</th>
                            </tr>
                        </thead>
                        ...
                        {foreach from=$listAllInvoice item=show}
                            <tbody>
                                <tr>
                                    <td>{$show.total|@number_format:2:".":","} $</td>
                                    <td>{$show.pay_date}</td>
                                    <td>{$show.note}</td>
                                    <td>
                                        <!-- edit branch -->
