...
<div class="container" style="width: 140mm;height:auto;padding-top:0;">
  <div class="row">
    ...
    <table class="table table-striped" width="100%" style="margin-bottom:0px">
      ...
      <tr>
        <td>{$k + 1}</td>
        <td>{$product.title} / {$product.product_color_name} / {$product.product_storage_name} / {$product.imei}</td>
        <td>1</td>
        <td>$ {math equation="x * y" x=$product.quantity y=$product.price format="%.2f"}</td>
        <td style="text-align: right;">$ {math equation="x * y" x=$product.quantity y=$product.price format="%.2f"}</td>
      </tr>
      {/foreach}
    </table>
    <hr style="margin-bottom:5px;margin-top:0px;border: 1px solid;" />
